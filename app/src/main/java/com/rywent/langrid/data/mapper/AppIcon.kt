...
enum class AppIcon(val key: String, val icon: ImageVector) {
    ...
    companion object {
        fun fromKey(key: String): ImageVector {
            return entries.firstOrNull { it.key == key }?.icon
                ?: entries.firstOrNull { it.icon.name == key }?.icon
                ?: QUESTION_MARK.icon
        }

        fun fromIcon(icon: ImageVector): String {
            return entries.firstOrNull { it.icon == icon }?.key
                ?: entries.firstOrNull { it.icon.name == icon.name }?.key
                ?: entries.firstOrNull { it.key.equals(icon.name.substringAfterLast('.'), true) }?.key
                ?: "QuestionMark"
        }
    }
}